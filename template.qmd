...
```{r setup}
# r setup gives code chunk name in nav 
...
summary measure of average achievement in key dimensions of human development: a long and healthy life, being knowledgeable and having a decent standard of living. The HDI is the geometric mean of normalized indices for each of the three dimensions.
...
```{r}
hdi_raw <- read_csv(here("data", "hdi_clean.csv"))

mod_hdi_long <- hdi_raw |> pivot_longer(names_to='name', values_to = 'value', where(is.numeric))

ggplot(mod_hdi_long, aes(x = value)) +
  geom_histogram() +
  facet_wrap(~ name, scales = 'free_x')

hdi_data <- hdi_raw |> drop_na() |>
...
looks like pc1 and pc2 describe most of the data
...
ggplot() + geom_col(pct_expl_df, mapping = aes(x=pc, y=pct_v)) + labs(ylab("Percent")) + scale_y_continuous(labels=scales::percent, expand=c(0,0))
